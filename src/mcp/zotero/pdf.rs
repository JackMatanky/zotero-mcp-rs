...
use crate::{
    ...
    zotero::{ItemKey, ItemType, LinkMode, ZoteroClient, ZoteroItem},
};
...
impl ZoteroMcpServer {
    ...
    pub(in crate::mcp::zotero) async fn fetch_bridge_pdf_roots(
        &self,
    ) -> Vec<BridgePdfRoot> {
        let url = format!(
            "{}{}",
    ...
    ///   byte limits.
    pub(in crate::mcp::zotero) fn validate_pdf_read_path(
        &self,
        path: &Path,
        bridge_roots: &[BridgePdfRoot],
        direct_input: bool,
