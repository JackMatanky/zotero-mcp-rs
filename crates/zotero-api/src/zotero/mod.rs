...
pub use objects::{LocalApiStatus, ZoteroCollection, ZoteroItem};
pub use search::{
...
pub mod test_http {
    ...
    impl MockServer {
        ...
        fn with_log(
            ...
            #[expect(
                clippy::expect_used,
                reason = "test-only mock server; panicking on bind failure is \
                          intended"
            )]
            let listener =
    ...
    fn record_or_drain_request(
        ...
        if let Some(recorded) = recorded {
            #[expect(
                clippy::expect_used,
                reason = "test-only mock server; a poisoned lock indicates a \
                          panicked test thread, so panicking here is intended"
            )]
    ...
    #[expect(
        ...
        reason = "test-only request-body JSON parser; failure is just \
                  propagated serde_json::Error"
    )]
    #[inline]
    pub fn request_body(
        raw: &str,
