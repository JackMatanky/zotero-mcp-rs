...
| **# tools** | **56** | **~30** | 20 | search-focused, read-only | 15 | 5 | 5 |
...
| **Search** | quicksearch, by tag, by citekey; advanced ✅ whole-library (server pushdown + client filter), operators/join/sort, paginated `{items, pagination}` | ✅ keyword, advanced multi-criteria, tags, citekey, notes/annotations | ✅ boolean, filters, relevance, offsets | ✅ hybrid semantic+keyword (RRF), multi-query AND/OR, section-aware, passage previews | ✅ by query or sorted field | citekey lookup only | author/title only, 30 items |
...
| **Duplicates / analytics** | ✅ find_duplicates, library_coverage (whole-library scan) | ✅ find **+ merge** duplicates (consolidates children, dry-run) | ❌ | ❌ | ❌ | ❌ | ❌ |
...
Rust MCP server (rmcp, 56 tools) speaking to Zotero's **Local HTTP API** with optional Better BibTeX (JSON-RPC) and Better Notes (companion bridge) integrations. Read-only by default (`ZOTERO_WRITE_ENABLED=1` opts into 22 write tools); hardened path allowlists (4 security profiles), response size caps, and symlink-safe canonicalization. Strengths: write breadth (tags, notes, collections, annotations, item lifecycle, delete/trash/restore), BBT citation toolchain (citekeys, bibliography, pandoc filter, scanAUX, autoexport), identifier ingestion (DOI/arXiv/ISBN), whole-library advanced search + duplicates and library-coverage analytics, local sqlite full-text + note/annotation search. Limitations: local user library only (no web API/groups), no semantic search, no PDF upload, no `.docx` injection.
...
1. ~~**Server-side advanced search / pagination**~~ **CLOSED** — whole-library scans (no 100-item ceiling), server-side pushdown, paginated `{items, pagination}` results; plus two gated sqlite-backed tools `zotero_fulltext_search` / `zotero_search_notes_annotations` for full-text + note/annotation search.
